...
<template>
  <div class="flex-1 flex flex-col h-full overflow-hidden bg-mcp-dark/50 backdrop-blur-sm rounded-2xl border border-gray-800 shadow-2xl">
    <!-- Messages Area -->
      ...
      <div v-for="(msg, index) in chatStore.messages" :key="index" class="flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div v-if="msg.role === 'user'" class="self-end max-w-[85%] group">
          <div class="flex flex-col items-end space-y-1">
            <div class="bg-mcp-blue text-mcp-dark px-5 py-3 rounded-2xl rounded-tr-none font-medium shadow-lg shadow-mcp-blue/10">
              {{ msg.content }}
            </div>
        ...
        <div v-if="msg.role === 'agent'" class="self-start max-w-[85%] group">
          <div class="flex flex-col items-start space-y-1">
            <div class="bg-gray-800/80 text-gray-100 px-5 py-3 rounded-2xl rounded-tl-none border border-gray-700 shadow-xl backdrop-blur-md">
              <p class="whitespace-pre-wrap leading-relaxed text-sm selection:bg-mcp-blue selection:text-mcp-dark">{{ msg.content }}</p>
            </div>
        ...
        <div v-if="msg.role === 'thinking'" class="self-start pl-2 flex items-center space-x-3">
          <div class="flex space-x-1.5">
            <div class="w-1.5 h-1.5 bg-mcp-blue rounded-full animate-bounce [animation-delay:-0.3s]"></div>
            <div class="w-1.5 h-1.5 bg-mcp-blue rounded-full animate-bounce [animation-delay:-0.15s]"></div>
            <div class="w-1.5 h-1.5 bg-mcp-blue rounded-full animate-bounce"></div>
          </div>
          <span class="text-[10px] font-black text-mcp-blue uppercase tracking-[0.2em] animate-pulse">Processing</span>
        </div>
    ...
    <div class="p-6 bg-gray-900/50 border-t border-gray-800">
      <form @submit.prevent="handleSend" class="relative group">
        <input
          ...
          class="w-full bg-gray-950 border border-gray-700 text-gray-100 rounded-xl px-5 py-4 pr-24 focus:outline-none focus:ring-2 focus:ring-mcp-blue/50 focus:border-mcp-blue transition-all duration-300 placeholder:text-gray-700 font-mono text-sm tracking-wide shadow-inner"
          :disabled="chatStore.isThinking"
        >
        <div class="absolute right-2 top-2 bottom-2 flex items-center">
          <button
            type="submit"
            class="h-full bg-mcp-blue hover:bg-blue-400 disabled:bg-gray-800 disabled:text-gray-600 text-mcp-dark px-6 rounded-lg font-black text-xs uppercase tracking-widest transition-all duration-200 active:scale-95 shadow-lg shadow-mcp-blue/20"
      ...
      <div class="mt-3 flex justify-between items-center px-1">
        <div class="flex items-center space-x-4">
          <div class="flex items-center space-x-1.5">
            <div class="w-1 h-1 rounded-full" :class="chatStore.isThinking ? 'bg-mcp-yellow animate-pulse' : 'bg-gray-700'"></div>
            <span class="text-[9px] font-bold text-gray-600 uppercase tracking-tighter">System Engine</span>
          </div>
          <div class="flex items-center space-x-1.5">
            <div class="w-1 h-1 rounded-full" :class="!chatStore.isThinking ? 'bg-mcp-green' : 'bg-gray-700'"></div>
