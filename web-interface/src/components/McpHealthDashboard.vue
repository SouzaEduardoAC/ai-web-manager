  ...
  <div class="space-y-6">
      ...
      <div class="bg-gray-800 rounded-lg p-4 border border-gray-700 shadow-lg">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-mcp-blue">{{ mcpStore.summary.total }}</p>
          </div>
          <div class="text-mcp-blue">
      ...
      <div class="bg-gray-800 rounded-lg p-4 border border-green-700/50 shadow-lg">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-mcp-green">{{ mcpStore.summary.healthy }}</p>
          </div>
          <div class="text-mcp-green">
            <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-gray-800 rounded-lg p-4 border border-red-700/50 shadow-lg">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-mcp-red">{{ mcpStore.summary.unhealthy + mcpStore.summary.reconnecting }}</p>
          </div>
          <div class="text-mcp-red">
            <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-gray-800 rounded-lg border border-gray-700 shadow-xl">
      ...
      <div v-if="mcpStore.isLoading && !mcpStore.registry.length" class="p-12 text-center text-gray-400">
        <div class="animate-spin h-8 w-8 border-4 border-mcp-blue border-t-transparent rounded-full mx-auto mb-4"></div>
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-sm text-gray-400">
                ...
                <div v-if="mcp.consecutiveFailures > 0" class="col-span-full text-mcp-yellow flex items-center space-x-1 mt-1">
                  <span class="text-lg leading-none">⚠</span>
                  <span>Consecutive Failures: {{ mcp.consecutiveFailures }}</span>
                </div>
                <div v-if="mcp.error" class="col-span-full text-mcp-red text-xs mt-1 p-2 bg-red-900/20 rounded font-mono break-all">
                  Error: {{ mcp.error }}
                </div>
              </div>
            </div>
            <div class="ml-4 opacity-0 group-hover:opacity-100 transition">
              <button @click="$emit('remove', mcp.name)" class="p-2 text-gray-400 hover:text-mcp-red transition" title="Remove MCP">
