...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-4">
    ...
    <div>
      ...
      <input v-model="form.name" type="text" required placeholder="my-mcp-server" class="w-full bg-gray-900 border border-gray-700 rounded-md px-3 py-2 text-white focus:ring-2 focus:ring-mcp-blue focus:border-transparent">
    </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-2 gap-3">
        <button v-for="type in transportTypes" :key="type.value" type="button" @click="form.transport = type.value"
          :class="form.transport === type.value ? 'border-mcp-blue bg-mcp-blue/10 ring-1 ring-mcp-blue' : 'border-gray-700 bg-gray-900'"
    ...
    <div v-if="form.transport === 'http' || form.transport === 'sse'" class="space-y-4">
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          ...
          <button type="button" @click="addHeader" class="text-xs text-mcp-blue hover:underline">+ Add Header</button>
        </div>
    ...
    <div class="flex space-x-3 pt-4">
      ...
      <button type="submit" class="flex-1 px-4 py-2 bg-mcp-blue hover:bg-blue-500 text-mcp-dark font-bold rounded-lg transition">Add Server</button>
    </div>
  </form>
</template>
