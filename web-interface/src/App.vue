...
<template>
  <div v-if="isInitializing" class="h-screen w-full flex flex-col items-center justify-center bg-mcp-dark text-white font-mono">
    <div class="relative">
      <div class="animate-spin h-16 w-16 border-4 border-mcp-blue/20 border-t-mcp-blue rounded-full mb-6"></div>
      <div class="absolute inset-0 flex items-center justify-center">
        <div class="w-2 h-2 bg-mcp-blue rounded-full animate-pulse"></div>
      </div>
    </div>
    <p class="tracking-[0.3em] uppercase text-[10px] text-mcp-blue font-black animate-pulse">Initializing System Core</p>
  </div>

  <div v-else class="h-screen w-full flex flex-col bg-mcp-dark text-gray-100 font-sans selection:bg-mcp-blue selection:text-mcp-dark">
    <!-- Top Navigation Bar -->
    <nav class="h-16 border-b border-gray-800 bg-gray-950/50 backdrop-blur-md flex items-center justify-between px-6 z-10">
      <div class="flex items-center space-x-8">
        <div class="flex items-center space-x-3">
          <div class="w-8 h-8 bg-mcp-blue rounded-lg flex items-center justify-center shadow-lg shadow-mcp-blue/20">
            <svg class="w-5 h-5 text-mcp-dark" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="flex space-x-1">
          <router-link to="/chat" 
            active-class="bg-gray-800 text-mcp-blue"
      ...
      <div class="flex items-center space-x-6">
        <div v-if="chatStore.availableModels.length > 0" class="hidden md:flex items-center space-x-3 bg-gray-900/50 px-3 py-1.5 rounded-lg border border-gray-800">
          ...
          <select v-model="chatStore.selectedModel" class="bg-transparent text-xs font-bold text-mcp-blue focus:outline-none cursor-pointer">
        ...
        <div class="flex items-center space-x-3 border-l border-gray-800 pl-6">
          ...
          <button @click="authStore.logout" class="p-2 text-gray-500 hover:text-mcp-red transition">
    ...
    <footer class="h-8 border-t border-gray-800 bg-gray-950 flex items-center justify-between px-6">
      <div class="flex items-center space-x-6">
        <div class="flex items-center space-x-2">
          ...
          <span class="text-[9px] font-bold text-mcp-blue font-mono">v1.0-JSON-RPC</span>
        </div>
        <div class="flex items-center space-x-2">
          <span class="text-[9px] font-black text-gray-700 uppercase tracking-widest">Latency</span>
          <span class="text-[9px] font-bold text-mcp-green font-mono">14ms</span>
        </div>
      </div>
      <div class="flex items-center space-x-2">
        <span class="text-[9px] font-black text-gray-700 uppercase tracking-widest">Encryption</span>
        <span class="text-[9px] font-bold text-mcp-yellow uppercase">AES-256-GCM</span>
      </div>
    </footer>
  </div>
</template>
